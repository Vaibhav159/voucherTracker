...
export default function Favorites() {
    ...
    return (
        <div className="flex flex-1 overflow-hidden relative bg-theme-bg">
            {/* Main Content */}
            <main className="flex-1 overflow-y-auto bg-theme-bg scroll-smooth">
                <div className="mx-auto max-w-[1200px] px-4 py-8 md:px-8 lg:py-10">
                    {/* Header */}
                    <div className="mb-10 flex flex-col gap-6 border-b border-copper/20 pb-8 md:flex-row md:items-end md:justify-between sticky top-0 z-10 bg-theme-bg pt-4 -mt-4">
                        <div>
                            <h1 className="text-3xl font-black text-warm-white md:text-4xl">
                                Your <span className="text-accent">Favourites</span>
                            </h1>
                            <p className="mt-2 text-lg text-accent-dim">
                                {hasAnyFavorites
                        ...
                        <div className="flex gap-1 overflow-x-auto rounded-lg bg-theme-surface p-1 border border-white/5 shadow-inner hide-scrollbar">
                            {tabs.map((tab) => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`rounded-md px-5 py-2 text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab
                                        ? 'bg-primary text-theme-primary font-bold shadow-sm'
                                        : 'text-accent-dim hover:bg-white/5 hover:text-primary'
                                        }`}
                                >
                                    {tab}
                                </button>
                    ...
                    {!hasAnyFavorites && (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <div className="size-24 rounded-full bg-theme-surface border border-white/5 flex items-center justify-center mb-6">
                                <span className="material-symbols-outlined text-accent-dim text-[48px]">favorite</span>
                            </div>
                            <h2 className="text-2xl font-bold text-warm-white mb-3">No Favorites Yet</h2>
                            <p className="text-accent-dim max-w-md mb-8">
                                Browse our vouchers, cards, and guides to start building your personalized collection of favorites.
                            </p>
                            <div className="flex gap-4">
                                <Link to="/vouchers" className="px-6 py-3 rounded-lg bg-primary text-theme-primary font-bold text-sm hover:bg-primary-hover transition-colors">
                                    Explore Vouchers
                                </Link>
                                <Link to="/cards" className="px-6 py-3 rounded-lg border border-copper/30 text-accent-secondary font-bold text-sm hover:bg-copper/10 transition-colors">
                                    Browse Cards
                                </Link>
                            </div>
                        </div>
                        ...
                        <section className="mb-12">
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {favoriteVouchers.map((voucher) => (
                                    <div key={voucher.id} className="group relative overflow-hidden rounded-xl border border-white/5 bg-theme-surface p-5 transition-all hover:border-primary/50 hover:shadow-lg">
                                        <div className="mb-2 flex justify-between items-center">
                                            <span className="inline-flex items-center rounded-md bg-white/5 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-accent-dim">Voucher</span>
                                            <button
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-theme-border ${voucher.color || 'text-accent'}`}>
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <p className="text-xs font-medium text-accent-dim">{voucher.category}</p>
                                            </div>
                                        </div>
                                        <div className="mt-5 flex items-baseline gap-2">
                                            ...
                                            <span className={`flex items-center text-xs font-bold px-1.5 py-0.5 rounded ${voucher.trendType === 'up' ? 'text-green-500 bg-green-500/10' :
                                                voucher.trendType === 'down' ? 'text-red-500 bg-red-500/10' :
                                                    'text-accent-dim bg-accent-dim/10'
                                                }`}>
                                        ...
                                        <p className="mt-2 text-[11px] text-accent-dim">Real-time market rate</p>
                                        <div className="mt-5 flex gap-3">
                                            ...
                                            <button className="flex-1 rounded-lg bg-primary py-2.5 text-xs font-bold uppercase tracking-wider text-theme-primary transition-transform hover:scale-[1.02]">Buy Now</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <section className="mb-12">
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {favoriteCards.map((card) => (
                                    <div key={card.id} className="group relative overflow-hidden rounded-xl border border-white/5 bg-theme-surface p-0 transition-all hover:border-primary/50 hover:shadow-lg">
                                        <div className="relative h-40 p-5" style={{ background: `linear-gradient(to bottom right, ${card.bgFrom}, ${card.bgTo})` }}>
                                            <div className="relative z-10 flex h-full flex-col justify-between">
                                                <div className="flex justify-between">
                                                    <span className="text-sm font-bold tracking-widest text-theme-primary/90">{card.bank}</span>
                                                    <span className="material-symbols-outlined text-theme-primary/50">contactless</span>
                                                </div>
                                                <div>
                                                    <div className="mb-2 text-xs text-theme-primary/50">{card.name.toUpperCase()}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="p-5">
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    <span className="inline-flex items-center rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent-dim mb-1">Credit Card</span>
                                                    <h3 className="font-bold text-warm-white">{card.name}</h3>
                                                    <p className="text-xs text-accent-dim">{card.tier}</p>
                                                </div>
                                                <button
                                                    className="flex h-8 w-8 items-center justify-center rounded-full text-red-500 transition-colors hover:bg-red-500/10"
                                                    title="Remove from Favourites"
                                            ...
                                            <div className="mt-4 space-y-3">
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-accent-dim">Reward Rate</span>
                                                    <span className="font-bold text-primary">{card.rewardRate}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-accent-dim">Annual Fee</span>
                        ...
                        <section className="mb-12">
                            ...
                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {favoriteBanking.map((tier) => (
                                    <div key={tier.id} className="group relative flex flex-col overflow-hidden rounded-xl border border-white/5 bg-theme-surface p-5 transition-all hover:border-primary/50 hover:shadow-lg">
                                        <div className="mb-4 flex justify-between items-start">
                                            <div className="flex gap-4 items-center">
                                                <div
                                                    className="flex h-12 w-12 items-center justify-center rounded-lg text-theme-primary font-bold text-[10px] tracking-tighter"
                                                    style={{ backgroundColor: tier.bankColor }}
                                                >
                                                    {tier.bank}
                                                </div>
                                                <div>
                                                    <span className="inline-flex items-center rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent-dim mb-1">Banking Tier</span>
                                                    <div className="flex flex-col">
                                                        <p className="text-[10px] font-bold uppercase tracking-wider text-accent-dim">{tier.tierType}</p>
                        ...
                        <section className="mb-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {[...favoriteGuides, ...favoriteArticles].map((guide) => (
                                    <div key={guide.id} className="flex flex-col gap-4 rounded-xl border border-white/5 bg-theme-surface p-4 transition-all hover:border-primary/30 sm:flex-row">
                                        <div className="aspect-video w-full overflow-hidden rounded-lg bg-theme-surface sm:w-40 shrink-0 flex items-center justify-center">
                                            <span className="material-symbols-outlined text-4xl text-theme-muted">article</span>
                                        </div>
                                        <div className="flex flex-1 flex-col justify-between">
                                            ...
                                            <div className="flex items-center justify-between mt-3">
                                                <div className="flex items-center gap-2 text-xs text-accent-dim">
                                                    <span className="material-symbols-outlined text-[14px] text-accent-secondary">schedule</span>
                                                    {guide.readTime}
                                                </div>
                                                <Link to="/guides" className="text-xs font-bold text-accent-dim hover:text-primary">Read Now</Link>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <footer className="mt-20 border-t border-copper/20 py-10 text-center">
                        <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                            <Link className="text-sm text-accent-dim hover:text-primary" to="/privacy">Privacy Policy</Link>
                            <Link className="text-sm text-accent-dim hover:text-primary" to="/terms">Terms of Service</Link>
                            <Link className="text-sm text-accent-dim hover:text-primary" to="/contact">Contact Support</Link>
                        </div>
                        <p className="mt-8 text-sm text-theme-muted">© 2024 CardPerks. All rights reserved.</p>
                    </footer>
                </div>
            </main>
        </div>
